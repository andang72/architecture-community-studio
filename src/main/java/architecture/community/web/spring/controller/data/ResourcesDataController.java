...
public class ResourcesDataController extends AbstractResourcesDataController {
	...
	@RequestMapping(value = "/data/resources/{objectType:[\\p{Digit}]+}/{objectId:[\\p{Digit}]+}/files", method = {
	...
	// 		RequestMethod.GET }, produces = MediaType.APPLICATION_JSON_VALUE)
	// @ResponseBody
	// public ItemList getFilesByObjectTypeNameAndObjectId(
	// 		@PathVariable Optional<String> objectTypeName,
	// 		@PathVariable Optional<Long> objectId,
	// 		@RequestParam(value = "fields", defaultValue = "none", required = false) String fields) {
	// 	ItemList result = new ItemList();
	// 	if (objectId.isPresent() && objectTypeName.isPresent()) {

	// 		Models model = Models.valueOf(objectTypeName.get().toUpperCase());
	// 		boolean includeLink = org.apache.commons.lang3.StringUtils.contains(fields, "link");
	// 		boolean includeTags = org.apache.commons.lang3.StringUtils.contains(fields, "tags");

	// 		DataSourceRequest dataSourceRequest = new DataSourceRequest();
	// 		dataSourceRequest.getParameters()
	// 				.add(new ParameterValue(0, "OBJECT_TYPE", Types.INTEGER, model.getObjectType()));
	// 		dataSourceRequest.getParameters().add(new ParameterValue(1, "OBJECT_ID", Types.NUMERIC, objectId.get()));
	// 		dataSourceRequest.setStatement("COMMUNITY_WEB.COUNT_ATTACHMENT_BY_OBJECT_TYPE_AND_OBJECT_ID");
	// 		int totalCount = customQueryService.queryForObject(dataSourceRequest, Integer.class);
	...
	public List<Attachment> uploadFiles(
		...
		while (names.hasNext()) {
			...
			if (attachmentId > 0) {
				...
				attachment = attachmentService.createAttachment(objectType, objectId, mpf.getOriginalFilename(),
			...
			if (shared) {
				SharedLink link = sharedLinkService.getSharedLink(Models.ATTACHMENT.getObjectType(),
						attachment.getAttachmentId(), shared);
	...
	@RequestMapping(value = "/data/files/{attachmentId:[\\p{Digit}]+}/get.json", method = { RequestMethod.POST,
			RequestMethod.GET })
	@ResponseBody
	...
	@RequestMapping(value = "/data/files/{attachmentId:[\\p{Digit}]+}/delete.json", method = { RequestMethod.POST,
			RequestMethod.GET })
	@ResponseBody
	...
	public ItemList uploadImages(
		...
		while (names.hasNext()) {
			...
			if (imageId > 0) {
				...
				image = imageService.createImage(objectType, objectId, mpf.getOriginalFilename(), mpf.getContentType(),
						is, (int) mpf.getSize());
			}
			image.setUser(user);
	...
	@RequestMapping(value = "/data/images/{imageId:[\\p{Digit}]+}/get.json", method = { RequestMethod.POST,
			RequestMethod.GET })
	@ResponseBody
	...
	@RequestMapping(value = "/data/images/{imageId:[\\p{Digit}]+}/delete.json", method = { RequestMethod.POST,
			RequestMethod.GET })
	@ResponseBody
