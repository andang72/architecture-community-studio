  ...
  <script>		
	require.config({
  	...
  	require([ "jquery", "bootstrap", "hs.side-nav", "hs.hamburgers" , "hs.dropdown" , "jquery.scrollbar", "hs.scrollbar" , "hs.popup", "studio.custom" ], function($) {  
			...
			fields: { 	
				enabled : { type : "boolean", defaultValue: ${ CommunityContextHelper.getConfigService().getApplicationBooleanProperty( CommunityConstants.SERVICES_AUDIT_ENABLED_PROP_NAME, false)?string} } 
			},
		...
		setSource : function( data ){
			...
			createAuditTrailGrid($this);	
		},
		load: function(){
			var $this = this;
			community.ui.progress($('#features'), true);	
			community.ui.ajax('<@spring.url "/data/secure/mgmt/services/audit/config.json"/>', {
		...
		saveOrUpdate : function(){
			...
			var dialog = community.ui.dialog( null, {
                ...
                { text: '확인', 
                	action: function(e){  
                		community.ui.progress($('#features'), false);   
						community.ui.ajax( '<@spring.url "/data/secure/mgmt/services/audit/config/save-or-update.json" />', {
 	...
 	function createAuditTrailGrid( observable ) {
  		var renderTo = $('#audit-trail-grid');
		if( !community.ui.exists(renderTo) ){   
			community.ui.grid(renderTo, {
				autoBind : false,
				dataSource: {
					transport: { 
						read : { url:'<@spring.url "/data/secure/mgmt/services/audit/list.json"/>', type:'post', contentType: "application/json; charset=utf-8"},
				...
				columns: [
					...
					{ field: "AUD_RESOURCE", title: "RESOURCE" ,  width: 100  },
          	...
          	<header class="card-header g-brd-gray-light-v7 g-bg-transparent g-px-15 g-px-30--sm g-pt-15 g-pt-20--sm g-pb-10 g-pb-15--sm g-mb-5">
					<div class="media">
						...
						<div class="media-body d-flex justify-content-end"> 
							...
							<a class="d-flex align-items-center u-link-v5 g-color-gray-light-v6 g-color-secondary--hover g-ml-10 g-ml-15--sm g-ml-30--xl" href="#" data-bind="click:refresh" >
								<i class="hs-admin-reload g-font-size-18"></i>
							</a>
						</div>
					</div>
			</header>
            <div id="audit-trail-grid" class="g-brd-left-0 g-brd-right-0 g-mb-1">
         </div>
          <!-- End Content --> 
        </div>
	...
	<div class="g-pa-15 g-pa-30--sm"> 
		...
		<div class="row no-gutters">
			...
			<div class="col-md-9 ml-auto text-right" data-bind="visible:editable" style="display:none;">  
				<button class="btn btn-md btn-xl--md g-rounded-50 u-btn-secondary g-width-120--md g-font-size-12 g-font-size-default--md g-mr-10 " data-bind="click:saveOrUpdate" type="button">저장</button>
				<button class="btn btn-md btn-xl--md g-rounded-50 u-btn-outline-gray-dark-v6 g-font-size-12 g-font-size-default--md " type="button" data-bind="click:cancle" >취소</button>
			</div>
		</div> 
	</div>
  </div>  
