			...
			var dialog = community.ui.dialog( null, {
                ...
                { text: '확인', 
                	action: function(e){ 
						...
						community.ui.ajax('<@spring.url "/data/secure/mgmt/resources/decorator/save-or-update.json"/>', {
  	...
  	function createTreeView(observable){
		...
		if( !community.ui.exists(renderTo) ){  
			...
			var treeview = community.ui.treeview ( renderTo , { 
				dataSource : {					
					transport: {
						read: { url : '<@spring.url "/data/secure/mgmt/resources/decorator/list.json"/>', dataType: "json" }
					},
					schema: {		
				...
				change: function(e) {
					...
					if( community.ui.defined(filePlaceHolder) ){ 
						community.ui.progress($('#features'), true );
						community.ui.ajax( '<@spring.url "/data/secure/mgmt/resources/decorator/get.json" />' ,{
