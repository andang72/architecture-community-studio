    ...
    function createApiGrid(observable){
		...
		if( !community.ui.exists(renderTo) ){  
			community.ui.grid(renderTo, {
				dataSource: {
					transport: { 
						read : { url:'<@spring.url "/data/secure/mgmt/apis/list.json"/>', type:'post', contentType: "application/json; charset=utf-8"},
				...
				{ field: "NAME", title: "페이지", filterable: false, sortable: true, template:$('#name-column-template').html() },   
				{ field: "VERSION_ID", title: "버전", filterable: false, sortable: false, width:120, template: '#= version#', attributes:{ class:"text-center" } }, 
				{ field: "PAGE_STATE", title:"사용여부", filterable: false, sortable: true, template: $('#enabled-column-template').html() , attributes:{ class:"text-center" } , width:100 },  
				{ field: "USER_ID", title: "작성자", filterable: false, sortable: true, width:100, template: $('#user-column-template').html(), attributes:{ class:"text-center" } },
			...
			$('#features').on( "click", "a[data-action=edit]", function(e){		
				...
				if( community.ui.defined($this.data("object-id")) ){
					var objectId = $this.data("object-id");	
  					community.ui.send("<@spring.url "/secure/studio/resource-apis-editor" />", { apiId: objectId });
  				}		
			});															
		}			
    } 
...
<body>
  <main class="container-fluid px-0 g-pt-65">
        ...
        <div class="g-pa-20">
          ...
          <!-- Content -->
			<div id="features" class="container-fluid"> 
				<div class="row"> 
					<div id="pages-grid" class="g-brd-0 g-mb-1"></div>
				</div>
			</div>
          <!-- End Content --> 
        </div>
	...
	<script type="text/x-kendo-template" id="name-column-template">    
		<a class="d-flex align-items-center u-link-v5 u-link-underline g-color-black g-color-lightblue-v3--hover g-color-lightblue-v3--opened" href="\#!" data-action="edit" data-object-id="#=apiId#">
	...
	<div class="media">
    	<div class="d-flex align-self-center">
    		<img class="g-width-36 g-height-36 rounded-circle g-mr-15" src="#= community.data.url.userPhoto( '<@spring.url "/"/>' , creator ) #" >
		</div>
