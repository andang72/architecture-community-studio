  ...
  <script>	
	...
	require.config({
		shim : {
	        ...
	        "hs.markup-copy" 				: { "deps" :['jquery', 'clipboard.min', 'hs.core'] },
  	...
  	require([ "jquery", "bootstrap", "hs.side-nav", "hs.hamburgers" , "hs.dropdown" , "jquery.scrollbar", "hs.scrollbar" , "hs.popup", "studio.custom",  "ace", "dropzone" , "imagesloaded"], 
		...
		image : new community.data.model.Image(),
		visible : false,
		editable : true,
		isNew : true, 
		linked : false,
		...
		load: function(objectId){
			...
			if( objectId > 0 ){
				...
				community.ui.ajax('<@spring.url "/data/secure/mgmt/images/"/>' + objectId + '/get.json?fields=imageLink,tags', {
					contentType : "application/json",
					success: function(data){	
						$this.setSource( new community.data.model.Image(data) );
					}	
				}).always( function () {
					community.ui.progress($('#features'), false);
				});	
			}else{
				$this.setSource(new community.data.model.Image()); 
			}	
		},	
		property : function(e){
			var $this = this;
			createPropertyWindowIfNotExist(community.data.Models.Image, $this.image.imageId );
		},
		permissions : function(e){
			var $this = this;
			createPermissionsWindowIfNotExist(community.data.Models.Image, $this.image.imageId); 
		},
		setSource : function( data ){
			...
			if( data.get('imageId') > 0 ){
				...
				$this.set('imageUrl', community.data.url.image (data, {thumbnail:false}) ); 
				if( $this.image.imageLink != null && $this.image.imageLink.linkId != null ){
					$this.set('linked', true );
					$this.set('linkedUrl', '<@spring.url "/download/images/" />' + $this.image.imageLink.linkId ); 
				}else{ 
					$this.set('linked', false );
					$this.set('linkedUrl' , null );
				} 
				createCommentListView( null, $this.image.imageId ); 
			}else{
				$this.set('editable', true );	
				$this.set('isNew', true );
			}
		...
		getImageLink : function(){ 
					...
					if( $this.image.imageId > 0 ){
						community.ui.progress($('#features'), true);
						community.ui.ajax( '<@spring.url "/data/secure/mgmt/images/" />' + $this.image.imageId + '/link.json?create=true', {
							data: community.ui.stringify($this.menu),
							contentType : "application/json",
							success : function(response){
								$this.image.imageLink = response ;
		...
		delete : function(e){
			var $this = this; 
			var template = community.data.template('"#: image.name #"를 영구적으로 제거합니다.<br/> 이동작은 최소할 수 없습니다.');
			var dialog = community.ui.dialog( null, {
                ...
                { text: '확인', 
                	action: function(e){  
                		...
                		community.ui.ajax( '<@spring.url "/data/secure/mgmt/images/" />'+ $this.image.imageId +'/delete.json', {
							data: community.ui.stringify({}),
							contentType : "application/json",
							success : function(response){ 
								dialog.close();
								community.ui.notify( "이미지가 삭제되었습니다.");
								community.ui.send('<@spring.url "/secure/studio/resource-images"/>');
							}
						}).always( function () {
		...
		saveOrUpdate : function(e){ 
			...
			var validator = community.ui.validator($("#pageForm"), {});
				community.ui.progress($('#features'), true);	
				community.ui.ajax( '<@spring.url "/data/secure/mgmt/images/save-or-update.json" />', {
	...
	function createImageDropzone( observable ){	 
		...
		var myDropzone = new Dropzone( renderTo, {
			url: '<@spring.url "/data/secure/mgmt/images/upload_image_and_link.json"/>',
			paramName: 'file',
			maxFiles: 1,
			...
			<div class="row"> 
				...
				<div class="card g-brd-gray-light-v7 g-brd-0 g-rounded-3 g-mb-30 g-min-height-500" data-bind="visible:visible" style="display:none;" >
					...
					<div class="row g-my-15">
						<div class="col-md-6"> 
							<figure class="g-pos-rel thumbnail" id="image-thumbnail" >
					...
					<div class="row g-mb-30" >
						<div class="col-md-6">
										...
										data-bind="value: image.objectType, enabled:editable, source: objectTypes"
										style="width: 100%;" /> 
									</div>
						</div>
						<div class="col-md-6">
		                			...
		                			<div class="form-group g-pos-rel g-rounded-4 mb-0">
					                    ...
					                    class="form-control form-control-md" type="number" data-min="-1" step="1"  data-format="###" data-bind="value:image.objectId, enabled:editable" style="width: 100%"/>
				                    </div>
						</div>	 
					</div>
					<section data-bind="invisible:isNew" >
					<label class="g-mb-10 g-font-weight-600">이미지 링크</label> 
					<div class="g-pos-rel g-mb-20 ">
					<button class="btn u-input-btn--v1 g-width-80 g-color-white g-bg-primary g-rounded-right-4" type="button" data-bind="invisible:linked, click: getImageLink" style="display: none;">
					링크생성
					</button>
					<input class="form-control form-control-md g-brd-gray-light-v7 g-brd-gray-light-v3 g-rounded-4 g-px-14 g-py-10" type="text" placeholder="링크" data-bind="value:image.imageLink.linkId,enabled:false" disabled="disabled">
					...
					<div class="form-group g-mb-15">
						<label class="g-mb-10 g-font-weight-600" for="input-image-name">이름 <span class="text-danger">*</span></label>
		                    		<div class="g-pos-rel">
		                      		...
		                      		<input id="input-image-name" name="input-image-name" class="form-control form-control-md g-brd-gray-light-v7 g-brd-lightblue-v3--focus g-rounded-4 g-px-14 g-py-10" type="text" placeholder="파일명을 입력하세요" 
		                      			data-bind="value: image.name, enabled:editable" required validationMessage="이름을 입력하여 주세요." autofocus>
		                    		</div>
						<span class="k-widget k-tooltip k-tooltip-validation k-invalid-msg" data-for="input-image-name" role="alert" style="display:none;"></span>
	                </div>
	                
	                <div class="row g-mb-15" >
	                	<div class="col-md-6">
							<div class="form-group g-mb-30">
	                    		<label class="g-mb-10 g-font-weight-600" for="input-image-contentType">콘텐츠 타입</label>
		                    	<div class="g-pos-rel">
		                    		<input  class="form-control h-100 form-control-md g-brd-gray-light-v7 g-brd-lightblue-v3--focus g-rounded-4 g-px-20 g-py-12" type="text" data-bind="value:image.contentType, enabled:false" /> 
			                    </div>
							</div>			
						</div>
						<div class="col-md-6">
							...
							<div class="g-pos-rel">
								<input  class="form-control h-100 form-control-md g-brd-gray-light-v7 g-brd-lightblue-v3--focus g-rounded-4 g-px-20 g-py-12" type="text" data-bind="value:image.size, enabled:false" data-format="##,#" />
							</div>
							</div>
						</div>		
					</div>
					...
					<div class="row g-mb-15" >
	                  			<div class="col-md-6">
									...
									<div class="g-pos-rel">
									<span data-bind="text:image.creationDate" data-format="yyyy.MM.dd HH:mm"/>
									</div> 
								</div>
								<div class="col-md-6">
									<label class="g-mb-10 g-font-weight-600">수정일</label>
									<div class="g-pos-rel">
									<span data-bind="text:image.modifiedDate" data-format="yyyy.MM.dd HH:mm"/>
									</div> 
								</div>
	                </div>
				...
				<#if RequestParameters.imageId?? > 
				<#assign COMMENT_OBJECT_TYPE = 11 />	
			    <#assign COMMENT_OBJECT_ID = ServletUtils.getStringAsLong(RequestParameters.imageId) />
				...
				<div class="g-brd-left--lg g-brd-gray-light-v4 col-lg-2 g-mb-10 g-mb-0--md"> 
					...
					<section data-bind="invisible:isNew" style="display:none;" class="g-mt-30 g-px-10"> 
						<h3 class="h6 g-font-weight-300  g-color-gray-dark-v1 g-mb-10">태그</h3>
						<ul class="list-inline g-font-size-12 mb-0">
							<li class="list-inline-item g-mb-10">
								<span class="u-label u-label--sm g-bg-gray-light-v4 g-color-main g-rounded-20 g-px-10" data-bind="text:image.tags"></span>
							</li>
						</ul>
					</section>
				</div>
